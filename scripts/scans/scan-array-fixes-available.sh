...
for IMAGE in "${images[@]}"; do
  ...
  output=$(grype $IMAGE -o json 2>/dev/null | jq -c '{
    Total: [.matches[] | select((.vulnerability.fix.state | ascii_downcase) == "fixed")] | length, 
    Critical: [.matches[] | select((.vulnerability.severity | ascii_downcase) == "critical" and (.vulnerability.fix.state | ascii_downcase) == "fixed")] | length, 
    High: [.matches[] | select((.vulnerability.severity | ascii_downcase) == "high" and (.vulnerability.fix.state | ascii_downcase) == "fixed")] | length, 
    Medium: [.matches[] | select((.vulnerability.severity | ascii_downcase) == "medium" and (.vulnerability.fix.state | ascii_downcase) == "fixed")] | length, 
    Low: [.matches[] | select((.vulnerability.severity | ascii_downcase) == "low" and (.vulnerability.fix.state | ascii_downcase) == "fixed")] | length
  }')
  echo "$output"
